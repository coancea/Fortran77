...
%%
Finally, Section~\ref{subsec:MotPred} motivates the approach taken in 
this paper that, rather than evaluating $D$ at runtime, continues the
...
and can be seen as a {\sc dag} in which the leaves correspond,
...
or (ii) control flow: gates predicating summary existence (prefixed by $\#$) 
or total ($_l\cup_{i=1}^N$) and partial ($_l\cup_{k=1}^{i-1}$) unions 
corresponding to a normalized loop $l$ of index {\tt i=\{1$\ldots$N\}}. 
%
