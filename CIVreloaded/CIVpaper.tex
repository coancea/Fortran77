...
%and possibly that the value of {\tt civ}
%after increment in some iteration is necessarily greater by at least
%one with respect to the values of {\tt civ} in all previous iterations.
%With this information, the values of {\tt civ} at the start
%of each iteration can be computed in parallel via a prefix-sum bulk operation.
...
%Finally, our technique is well integrated % fully implemented and 
%in a {\tt Fortran77} compiler that uses {\em hybrid analysis}, 
%in which data-sensitive invariants, such as privatization and 
%(loop) independence, which cannot be statically determined, are 
%modeled as equations on summaries, and their runtime evaluation\footnote{
%The common-verification path is optimized by factoring the
%summary equation into a cascade of sufficient-condition predicates 
%that are tested in the order of their complexities.   
%}%
%guards the parallel version of the code.  
...
and to query variables'  ranges. % value 
...
%call and control dependency graphs ({\sc cdg}) of a structured program 
%in {\sc ssa} form, where within a {\sc cdg} region nodes are parsed in 
%program order.   
%
...
% {\tt CORREC\_do401} and {\tt ACCTFOR\_do240}.
%
...
%\footnote{ It follows that a graph without back edges is acyclic.}
%It follows that a graph without back edges is acyclic.
...
%When the dec/increment is not a constant, a range is used: e.g. the {\sc veg}
%of $\mu$ node {\tt {\it CIV@1}} corresponding to loop {\tt EXTEND\_do400} 
%would have an edge between {\tt {\it CIV@1}} and {\tt {\it CIV@8}} of distance
%$[0,M]$ summarizing the evolution of {\tt {\it CIV@2}} in loop {\tt EXTEND\_do500}. 
...
%computing the cross-one-iteration
%distance between the values of the same {\sc civ},
...
%The latter is safe because the values of {\tt civ} are monotonically increasing
%within the loop, with step at most one, i.e., similar to summarizing
%across a loop of index between {\tt{}civ@1=0} and {\tt{}civ@5-1}. %with step $1$. 
...
%In practice, this commonly corresponds to the use of ``push-back'' 
%arrays, e.g., vectors whose elements are inserted at the end.  
...
in Section~\ref{sec:Background}. %, and denote such a variable by {\tt {\it CIV}}.
%
...
%in Figure~\ref{fig:EXTEND}. %and symbol {\tt {\it CIV}}.
...
{\sc civ} variable, which was given at the end of Section~\ref{sec:Background}.
%
...
%{\em First}, the indexes belonging to the 
%underestimate $\lfloor WF_{j}\rfloor$ do not result in cross dependencies, i.e.,
%$\lfloor WF_{j}\rfloor\mbox{~}\cap\mbox{~}\cup_{k=1}^{j-1}\lfloor WF_{k}\rfloor = \emptyset$.
%
%{\em Second}, all remaining indexes, $\lceil WF_{j}\rceil - \lfloor WF_{j}\rfloor = {\tt\{CIV@8\}}$,
%are overwritten by the next {\sc civ}-increasing iteration, i.e., \\
%{\tt{}CIV@8>CIV@1} $\Rightarrow$
%{\tt\{CIV@1\}} $\subseteq \lfloor WF_{j}\rfloor = {\tt[CIV@1,CIV@8-1]}$,
%
%where {\tt\{CIV@8\}} of the previous iteration was written as {\tt\{CIV@1\}} 
...
underestimate, 
%per-iteration {\sc wf} underestimate $\lfloor WF_j\rfloor$, 
...
%{\sc cdg}-transitive closure of all statement 
...
\section{Beyond Monotonicity} % : Privatizing Stack-Like Accesses
\label{sec:Stack}
...
\section{Appendix: Proof of the Theorem in Section~4} %~\ref{sec:Stack}
