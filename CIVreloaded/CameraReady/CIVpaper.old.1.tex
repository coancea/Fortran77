...
%and can be represented with the same interval because  
%{\tt civ$_\mu^i$+1>civ$_b^i$}, i.e., {\tt civ} is not updated,
%and so {\tt civ$_\mu^i$} equals {\tt civ$_b^i$}
...
Our technique is well integrated % fully implemented and 
...
We note that speedups of two of the benchmarks shown in this
paper were also reported in~\cite{CosPLDI}. They were obtained 
in part by using the CIV technique that is presented here for 
the first time. 
...
%
 (ii) there are no input nodes, and %no node in the associated {\sc veg} can be reached via any input nodes, and 
%
...
%to be associated with external nodes such as {\tt nxt@3} because {\tt nxt@3}
...
\subsection{Privatizing Stack-Like Accesses} % : Beyond Monotonicity 
\label{sec:Stack}
