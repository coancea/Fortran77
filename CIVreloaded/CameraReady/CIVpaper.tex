...
Analysis for automatic parallelization typically assumes that induction variables 
are well behaved, in that they can be (expressed and) substituted with closed-form 
expressions of the enclosing loop-nest indices.
%
This is especially important when induction variables are used inside 
array subscripts because dependence analysis~\cite{BanerjeeIneqTest,FeautrierDataflow,Pugh92theomega} 
usually requires that subscripts are affine expressions of the nest indices.
...
For example, the loop of index {\tt i} in Figure~\ref{fig:introEg}(a) increments 
variable {\tt k} by two in every iteration, and updates the element at index 
{\tt k} of array {\tt a}. %, i.e., {\tt a(k)}.
%
This uniform update enables the substitution of induction variable
{\tt k} with {\tt k0+2*i} in the body of the loop, %where {\tt i} is the loop index.
...
of {\tt k} produced by the previous iteration, and (ii) allows to easily determine
...
{\tt k0+2*i$_1$~=~k0+2*i$_2$ $\Rightarrow$ i$_1$=i$_2$}.
 ...
 to incrementing {\tt k} 
and (ii) enables a straightforward proof of the invariant 
...
generates the loop in Figure~\ref{fig:introEg}(b) 
...
Dependence analysis of array subscripts, 
for example for automatic parallelization, typically requires that array 
subscripts are affine expressions of the enclosing loop-nest indices.   
%

Induction variables that are updated in all iterations uniformely with the 
same amount in all places can be substituted with closed-formed formulas
in the nest indices     
