...
To make the presentation clearer we demonstrate it on loop {\tt EXTEND\_do400}
(not shown).
...
\begin{itemize}
    ...
    \item All remaining indices, are overwritten by the next 
        ...
        $\lceil WF_{i}\rceil - \lfloor WF_{i}\rfloor \subseteq \lfloor WF_{j} \rfloor$. %= {\tt\{civ@8\}}
...
%While we have studied so far the case when the {\sc civ} evolutions
...
level start from a cut-off point determined by the first iteration %of the {\tt while} loop 
...
%On the one hand, $\mathcal{F}$ uses the 
%{\sc civ}-summarized over and underestimates to derive sufficient-independence 
%predicates.
%%
...
\newblock {\em Int. Par. Arch. Comp. Tech. (PACT)}, pages 43--52, 2006.
